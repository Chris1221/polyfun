...
def munge_polyfun_sumstats(
    ...
    """
    This function reads in summary statistics from a file, performs some data cleaning and filtering, and returns a pandas dataframe.

    Parameters:
    sumstats (str): Path to the input summary statistics file or dataframe.
    out (str): Name of the output file.
    n (int): Sample size. If not specified, will try to infer this from the input file.
    min_info (float): Minimum INFO score (set to zero to avoid INFO-based filtering).
    min_maf (float): Minimum MAF (set to zero to avoid MAF-based filtering).
    remove_strand_ambig (bool): If True, strand-ambigous SNPs will be removed.
    chi2_cutoff (float): Chi2 cutoff for effective sample size computations.
    keep_hla (bool): If True, keep SNPs in the HLA region.
    no_neff (bool): If True, use the true rather than the effective sample size in BOLT-LMM runs.

    Returns:
    df_sumstats (pd.DataFrame): A pandas dataframe containing the cleaned and filtered summary statistics.
    """
    check_package_versions()
    configure_logger(out)

    # read sumstats file
    logging.info("Reading sumstats file...")
    t0 = time.time()
    df_sumstats = read(sumstats)
    ...
    # compute Neff
    if "CHISQ_BOLT_LMM" in df_sumstats.columns and not no_neff:
        if n is None:
            raise ValueError("--n must be specified with BOLT input files")
        Neff = compute_Neff(df_sumstats, n, chi2_cutoff)
    ...
    # write output
    logging.info(
        "Saving munged sumstats of %d SNPs to %s" % (df_sumstats.shape[0], out)
    )
    ...
    df_sumstats[columns_to_keep].to_parquet(out)
